...
def validate_block(env, block=None):
    ...
    delay = round(get_random_values(
        env.delays['block_validation'])[0], 4)
    return delay
...
def longest_chain(block, chain):
    if block.height > chain.head.height:
        ...
        print(
        f'{chain.node.nid} at {chain.env.now}: \
...
def heaviest_chain(block, chain):
    ...
    if block.parentHash == chain.head.hash:
        ...
        while next_main_chain_block != None:
            ...
            print(f'{chain.node.nid} at {chain.env.now}: \
            # change head from block {chain.head.hash} to \
            block {cur.hash}')
